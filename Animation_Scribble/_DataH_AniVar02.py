...
def animateF(values,varName,name,s=1,subj = (),scale = 1, tail = 1): #scale governs the scale ratio between the real and animated time, and tail governs the 'time length' of tail
    L,timestep = spl(values,varName)
    fig = plt.figure()
    axes = plt.gca()
    Lines = []
    Lines.append(axes.plot([], [], 'r-'))
    line, = Lines[0]
    print (type(line))
